...
| [`31-synthesis-perf.md`](./31-synthesis-perf.md) | **Perf synthesis.** The read-tier *pattern* is a keeper (adopt into `13`); the **Shen-on-LuaJIT perf bet is not** (JIT premise false → …). *(Superseded in part by `32`.)* Carries a panel-hardened serving-tier design (serve tokens, mlcache, kTLS, enforced as-of, trie policy matcher). | draft |
| [`32-shen-lua-perf-correction.md`](./32-shen-lua-perf-correction.md) | **CORRECTION (reading the actual artifact).** `pyrex41/shen-lua` is a JIT-engineered LuaJIT target (source→Lua→machine code; JIT-friendly value rep; native TCO; **native soa32 FFI-int32 inference engine, 8.9× / −93% alloc**; **~1.5× of SBCL**). agentzh's F1 is **refuted** for this impl. Perf is **no longer a Shen disqualifier**; the all-OCaml verdict was over-indexed on maturity/bus-factor priors that don't apply to the toolchain author. Security (`30`) + consistency (`29`) obligations still stand (language-agnostic). | draft |
