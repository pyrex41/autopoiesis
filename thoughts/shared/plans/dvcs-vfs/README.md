...
| [`26-hotpath-perf-architecture.md`](./26-hotpath-perf-architecture.md) | **Hot-path perf for the Shen path** (re-opens the expressiveness/provability-first branch). Crux: **proving is build-time, running is LuaJIT** — no provable-vs-fast trade. **One provable Shen source → compiled to the best host per path** (shen-lua/OpenResty for hot reads, shen-SBCL/go for lands). Brain decides, **nginx serves bytes zero-copy** (X-Accel/sendfile); content-addressing = zero-invalidation caching to the edge; EdenFS-shape (OpenResty = source-of-truth service, mount = thin client). | draft-for-review |
