...
| [`25-final-verdict-go-vs-ocaml.md`](./25-final-verdict-go-vs-ocaml.md) | **FINAL VERDICT: build all-OCaml** (`13`). shen-go's two headline advantages are false (you reuse ltx's ~200 LOC not the litefs daemon; go-git can't 3-way merge). Steal one idea: **policy-as-data (landed Datalog ruleset)**, not Shen/Prolog. Caveat (Ptacek): the defensible kernel is language-agnostic. Next: P0 of `13`, leading with the irmin-pack-concurrency gating spike. | draft |
