...
;; Open store and start system — everything below needs with-store
(format t "~%=== Starting system ===~%")
(autopoiesis.substrate:with-store ()
  (format t "Store opened.~%")

  ;; Create mutable agents visible to the frontend API
  (format t "~%=== Creating mutable agents (API-visible) ===~%")
  (handler-case
      (let ((agent-names '(("architect" . (:design :review :analyze))
                           ("coder"     . (:code :test :debug))
                           ("reviewer"  . (:review :security :analyze))
                           ("researcher" . (:search :analyze :report))
                           ("reasoner"  . (:logic :analyze :verify)))))
        (dolist (entry agent-names)
          (let ((agent (autopoiesis.agent:make-agent
                        :name (car entry)
                        :capabilities (cdr entry))))
            (autopoiesis.agent:register-agent agent)
            (format t "  Registered mutable agent: ~A (id: ~A)~%"
                    (autopoiesis.agent:agent-name agent)
                    (autopoiesis.agent:agent-id agent))))
        (format t "~D agents registered in API registry.~%"
                (length (autopoiesis.agent:list-agents))))
    (error (e) (format t "Mutable agent error: ~A~%" e)))

  ;; Create persistent agents for the Lisp demo (in-memory, no store needed)
  (format t "~%=== Creating persistent agents (demo) ===~%")
  ...
  ;; Load eval scenarios (needs store)
  ...
  ;; Start API server (inside with-store so it can query the store)
  ...
  ;; Keep alive (inside with-store so store stays open)
  (handler-case
      (loop (sleep 60))
