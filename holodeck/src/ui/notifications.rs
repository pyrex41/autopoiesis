...
pub fn collect_notifications(
    ...
    for ev in ev_snapshot_created.read() {
        queue.toasts.push(Toast::new(
            format!(
                "Snapshot created: {}",
                ev.snapshot.metadata.as_deref().unwrap_or(&ev.snapshot.id)
            ),
    ...
    for ev in ev_blocking.read() {
        queue.toasts.push(Toast::new(
            format!("Blocking request: {}", ev.request.prompt),
