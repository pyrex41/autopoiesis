...
5. **Orchestration Layer** (`packages/core/src/orchestration/`) - Conductor tick loop, timer heap, Codex CLI worker, substrate-backed event queue
6. **Integration + API Layer** (`packages/core/src/integration/`, `packages/api-server/src/`) - Codex bridge, MCP client, multi-provider agentic loops, REST/WebSocket (Clack/Woo), MCP server, SSE, JSON/MessagePack, Command Center endpoints (departments, goals, budgets, audit, approvals, evolution)
...
| 5 | Codex API integration | Complete |
...
- `orchestration-tests` - Conductor, timer heap, event queue, workers, Codex CLI (91 checks)
...
- `integration-tests` - Codex API, MCP, tools, events, agentic loops (649 checks)
...
- `bridge-protocol-tests` - Codex bridge protocol, message format (14 checks)
...
- `packages/core/docs/specs/06-integration.md` - Codex bridge, MCP integration
...
(run-Codex-cli config &key timeout on-complete on-error) ; spawn Codex CLI
